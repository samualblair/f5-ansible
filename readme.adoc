...
link:debug_notes[Troubleshooting FAQ]

== Description

This is part of a basic Collection of Playbooks, specifically for generating and pushing F5 Declarative Onboarding and F5 AS3 REST API based configurations, using Ansible to generate, and optionally push.
There are also some Ansible playbooks to make specific changes as well.
To aid in delivery of DO, AS3 declarations, or to prepare environments without ansible available, some Postman API collections are included as well.
...
. Update 'hosts.yml' accordingly for devices, this is YAML formatted example, can be swapped with INI formatted if desired. Most playbooks only need hostname to key in on, as IP will be taken from CSV file. There may be a few older playbooks still expecting IP (if hostname does not resolve in DNS). All other values can (and are generally expected) to come from CSV parsing.

. Update CSV variables file with detailed variables needed. New columns (variables) will automatically be detected (except in legacy folder playbooks, only kept for reference to alternative parsing method).
...
. You may (likely) need to change the python interpreter in the ansible.cfg file, you can try change from "interpreter_python = /usr/local/bin/python3" to maybe "interpreter_python = /usr/local/bin/python" or comment out to have system try and find the python installed.
...
- pb.do.wipeconf.yml (USE WITH CAUTION)
  ...
  This effectively will reset any DO, as if you have never configured device with DO before. This can be helpful to deal with configuration conflicts such as removing a vlan that previously existed (prior to first DO push). This will require entering the Configuration ID (will be prompted) as this is used in the DELETE message of the DO REST API.
...
== Dedicated DO Generation with Pyhon Script , bypassing Ansible Requirement
To simplify requirements , and help with conditions of Ansible Jinja Templating issues, a dedicated Python Generate script has been built.
This uses Python directly with Jinja2 to generate DO files, using the same source variables (CSV) file that would otherwise be used by the Ansible playbooks.

Included is a requirements.txt for easy standup of a Python venv (virtual environment) in case of needing to install python libraries for use.

Quick refresher for installing/setting up a Python venv and activating for use

```bash
# Can create a venv and folder with -m option in current folder
python3 -m venv .venv
# Environment can be activated
source .venv/bin/activate
# All requirements can now be installed - should cover all needs of this repo
pip install -r requirements.txt
# If additional installs are required they can also be added - in this example Jinja2 is the package
python3 -m pip install Jinja2
```

Quick refresher for setting up a Python venv and activating for use in the future

```bash
# To activate a virtual environment in python
For this repo:
source .venv/bin/activate
## From Bash , Linux or MacOS (from Bash)
source venv/bin/activate
## To activate from Git shell or Unix environments
source venv/Scripts/activate
## To activate a virtual environment in python (from powershell)
...
- Build out modular playbooks and templates (playbooks and templates that build configuration based on other playbooks/templates)


== API Collections

=== Several API Collections are included in this repo.
- Postman
  * Popular GUI first platform for API interaction, has focused heavily on online first.
  * I have seen this break when AWS is down, even when just trying to work locally!
- Hoppscotch
  * Open Source GUI first platform for API interaction, similar to postman, but focused on offline first.
  * Does not break when AWS is down!
- Powershell
  * Common scripting/shell environment found on almost any Windows machine, but installable cross platform.

=== The included basic API Collections and Environments may help with:
- Preparing of F5OS platforms using F5OS API

  This API is not declarative. Collection of F5OS REST API calls to configure base F5OS system, install and provision tenants Tenants (BIG-IP NEXT or BIG-IP Classic).

- Preparing of BIG-IP Classic using F5 (Classic) REST API
  
  This API is not declarative. Installing/Upgrading DO and/or AS3 packages on BIG-IP Classic. Performing any other initial configurations required.

- Delivering ready to use DO Declarations
- Delivering ready to use AS3 Declarations
- Interacting with F5 Distributed Cloud (XC) platform

  This API is not declarative. Configure many elements of a delivery network (DNS, LB, even remote CE devices).

- All of these Examples can be used as starting point for working with other REST APIs.
...
* 2026.05.30 - Added Mass AS3 Deployment PowerShell Script in API tools
* 2026.01.28 - Doc updates, Direct Python Generation updates
* 2026.01.21 - Added BIG-IP Hoppscotch REST API Collections
* 2025.10.31 - Added Powershell API Collections
* 2025.10.29 - Updated Secure Cipher examples
* 2025.09.05 - Added BIG-IP Postman REST Licensing
